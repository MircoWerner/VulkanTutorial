...
struct Vertex {
    ...
    getBindingDescription() { // vertex data interpretation: [vertex, vertex, ...]
    ...
    getAttributeDescriptions() { // per vertex data interpretation: vertex = [pos, color, texCoord]
...
private:
    ...
    std::vector<VkDescriptorSet> m_descriptorSets; // destroyed implicitly with the descriptor pool
...
//    };
    ...
    void recreateSwapChain() {
        ...
        while (width == 0 || height == 0) { // wait until window is not minimized anymore
            glfwGetFramebufferSize(m_window, &width, &height);
            glfwWaitEvents();
        }
...
//        }
    ...
    void createRenderPass() {
        ...
        colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR; // clear framebuffer before drawing a new frame (color and depth data)
        colorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE; // store rendered results in framebuffer (color and depth data)
        colorAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE; // (stencil data)
        colorAttachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE; // (stencil data)
        colorAttachment.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED; // do not care which layout the image was in previously
        colorAttachment.finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // multi sampled image

        VkAttachmentReference colorAttachmentRef{};
        colorAttachmentRef.attachment = 0;
        colorAttachmentRef.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // function as color buffer, this layout gives the best performance for that

        VkAttachmentDescription colorAttachmentResolve{}; // transfer multi sampled image to presentable image in the swap chain
        ...
        colorAttachmentResolve.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; // image to be presented in the swap chain
        ...
        VkSubpassDescription subpass{}; // render pass can have multiple subpasses (e.g. multiple postprocessing steps)
    ...
    void createDescriptorSetLayout() {
        ...
        uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT; // use in the vertex shader
    ...
    void createGraphicsPipeline() {
        ...
        VkPipelineVertexInputStateCreateInfo vertexInputInfo{}; // format of vertex data that is passed to the vertex shader
        ...
        VkPipelineInputAssemblyStateCreateInfo inputAssembly{}; // what kind of geometry is drawn from the vertices
        inputAssembly.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
        inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
        inputAssembly.primitiveRestartEnable = VK_FALSE;

//        VkViewport viewport{}; // region of the framebuffer the output is rendered to
//        viewport.x = 0.0f;
//        viewport.y = 0.0f;
...
//
//        VkRect2D scissor{}; // everything outside the scissor rectangle is discarded by the rasterizer
//        scissor.offset = {0, 0};
//        scissor.extent = m_swapChainExtent;
//
//        VkPipelineViewportStateCreateInfo viewportState{}; // set viewport and scissor as static part of the pipeline, pipeline recreation necessary for changes (e.g. resizing the window)
...
//        viewportState.pScissors = &scissor;
        ...
        VkPipelineViewportStateCreateInfo viewportState{}; // make dynamic viewport and scissor, is set up at drawing time
        viewportState.sType = VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO;
        viewportState.viewportCount = 1;
        viewportState.scissorCount = 1;
        ...
        rasterizer.depthClampEnable = VK_FALSE; // if true, clamps fragments to near or far plane if their depth is beyond the near or far plane, otherwise those fragments are discarded by the rasterizer
        rasterizer.rasterizerDiscardEnable = VK_FALSE; // if true, rasterizer discards all geometry
...
//                              VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, m_mipLevels); // transition now happens as part of the mipmap generation
    ...
    VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
        ...
        viewInfo.components.r = VK_COMPONENT_SWIZZLE_IDENTITY; // optional remapping of color channels
    ...
    void createVertexBuffer() {
        ...
        void *data;
        vkMapMemory(m_device, stagingBufferMemory, 0, bufferSize, 0, &data); // memory-mapped I/O
        ...
        copyBuffer(stagingBuffer, m_vertexBuffer,
                   bufferSize); // copy contents from staging buffer to high performance memory on GPU, which cannot be accessed directly by the CPU (therefore the staging buffer)
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        clearValues[1].depthStencil = {1.0f, 0}; // depth in [0,1] where 1 is at the far view plane
        ...
        VkRect2D scissor{}; // dynamic scissor state, has to be set in the command buffer
        scissor.offset = {0, 0};
        scissor.extent = m_swapChainExtent;
...
// MAIN LOOP
    void drawFrame() {
        vkWaitForFences(m_device, 1, &m_inFlightFences[m_currentFrame], VK_TRUE,
                        UINT64_MAX); // waiting for the previous frame to finish, blocks the CPU

        uint32_t imageIndex;
        VkResult result = vkAcquireNextImageKHR(m_device, m_swapChain, UINT64_MAX,
                                                m_imageAvailableSemaphores[m_currentFrame], VK_NULL_HANDLE,
                                                &imageIndex); // acquire image from the swap chain, send signal to semaphore when GPU finished, blocks all following calls to the GPU, does not block the CPU
        ...
        VkSemaphore waitSemaphores[] = {
                m_imageAvailableSemaphores[m_currentFrame]}; // submit the command buffer after the image is available (signal by the vkAcquireNextImageKHR(...) call)
        ...
        VkSemaphore signalSemaphores[] = {
                m_renderFinishedSemaphores[m_currentFrame]}; // is signaled when the command buffer has finished execution
        submitInfo.signalSemaphoreCount = 1;
        submitInfo.pSignalSemaphores = signalSemaphores;

        if (vkQueueSubmit(m_graphicsQueue, 1, &submitInfo, m_inFlightFences[m_currentFrame]) !=
            VK_SUCCESS) { // signal fence after the command buffer finished execution
            throw std::runtime_error("Failed to submit draw command buffer!");
        }

        VkPresentInfoKHR presentInfo{}; // present after the command buffer is executed
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.proj = glm::perspective(glm::radians(45.0f), m_swapChainExtent.width / (float) m_swapChainExtent.height,
                                    0.1f, 10.0f);
        ubo.proj[1][1] *= -1; // correction for y-axis
